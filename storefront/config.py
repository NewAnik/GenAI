...
DB credentials are fetched from Secrets Manager at cold start (cached via @lru_cache) when
DB_SECRET_ARN is set — this is what CDK wires up in production (see
infra/cdk/storefront_stack/storefront_stack.py), so rotating the secret doesn't require a
redeploy. Local dev/tests instead set DB_HOST/DB_PORT/DB_NAME/DB_USER/DB_PASSWORD directly
(no AWS credentials needed to run tests — see tests/storefront/conftest.py).
...
def _db_credentials_from_secret(secret_arn: str) -> dict:
    ...
    # Standard AWS-managed RDS credentials secret shape: {host, port, dbname, username, password}.
...
def get_settings() -> Settings:
    ...
    if db_secret_arn:
        creds = _db_credentials_from_secret(db_secret_arn)
        db_host = creds["host"]
