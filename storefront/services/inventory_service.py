"""Pure stock-reservation arithmetic over an `Inventory` row. Isolated from the DB so the
reserve/release/fulfill invariants are unit-testable; the caller is responsible for having
row-locked the inventory row (see InventoryRepository.lock_for_variant) before mutating it,
