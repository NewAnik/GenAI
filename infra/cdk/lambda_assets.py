...
def parse_handler(handler_spec: str) -> str:
    ...
    — the module path is relative to STOREFRONT_ENTRY (the code asset root), so the leading
    "storefront/" segment is stripped and the rest becomes dotted, as the generic
    `aws_lambda.Function` handler string requires."""
...
def build_dependencies_layer(scope: Construct, construct_id: str = "StorefrontDependencies") -> _lambda.LayerVersion:
    ...
    return _lambda.LayerVersion(
        ...
        description="storefront/requirements.txt dependencies",
    )
